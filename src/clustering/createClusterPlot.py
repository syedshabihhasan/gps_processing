...
def gettexts(template_start, template_end, rectangle_t_script, rectangle_s_script, marker_javascript):
    return readtemplatefile(template_start), readtemplatefile(template_end), readtemplatefile(
        rectangle_t_script), readtemplatefile(rectangle_s_script), readtemplatefile(marker_javascript)


def createclusterplot(path_to_use, stationary_clusters, travel_clusters, noise_markers, rectangle_t_script,
                      rectangle_s_script, marker_javascript, template_start, template_end, stationary_cluster_label):
    start_txt, end_txt, rectangle_t_txt, rectangle_s_txt, marker_txt = gettexts(template_start, template_end,
                                                                                rectangle_t_script, rectangle_s_script,
                                                                                marker_javascript)
    stationary_full_text = ''
    markers_stationary_info = '\nvar markers_stationary_info = ['
    markers_stationary = '\nvar markers_stationary = ['
    idx = 0
    for c_idx in range(len(stationary_clusters)):
        cluster = stationary_clusters[c_idx]
        cluster_label = stationary_cluster_label[c_idx]
